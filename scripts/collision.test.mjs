...
test('使用说明和每日碰撞次数只放首页，首次说明不在问题页触发',()=>{
  ...
  assert.match(home,/id="collisionQuota"/);
  assert.match(home,/data-onboarding-open/);
  assert.ok(home.indexOf('data-onboarding-open') < home.indexOf('id="collisionQuota"'));
  assert.doesNotMatch(question,/id="collisionQuota"|data-onboarding-open/);
  for(const page of [home,question]) assert.doesNotMatch(page,/class="zhida"|>.*直答.*<\/button>/);
  ...
  assert.match(site,/dataset\.page !== 'home'/);
  assert.match(site,/collision-onboarding-v2-home-only/);
